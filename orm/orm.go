...
func (o *orm) ShardingTable(sharding func(string) string) {
...
func (o *orm) Using(name string) error {
	...
	if al, ok := dataBaseCache.get(name); ok {
		...
		if Debug {
			o.db = newDbQueryLog(al, al.DB, o.sharding)
		} else {
			o.db = &DB{
...
func (o *orm) BeginTx(ctx context.Context, opts *sql.TxOptions) error {
	...
	o.isTx = true
	if Debug {
		o.db.(*dbQueryLog).SetDB(&TxDB{Tx: tx})
	} else {
		o.db = &TxDB{Tx: tx}
	}
	return nil
...
func NewOrmWithDB(driverName, aliasName string, db *sql.DB) (Ormer, error) {
	...
	if Debug {
		o.db = newDbQueryLog(o.alias, al.DB, o.sharding)
	} else {
		o.db = al.DB
	}
